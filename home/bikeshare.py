...
def get_filters():
    ...
    while True:
        city = input("Please choose the city you want to explore from the following list (chicago, new york city, washington) ?")
        city = city.lower()
    ...
    while True:
        month = input(
            "Do you want details specific to a particular month? If yes, please choose a month from the given list ("
            "all, january, february, ... , june)")
        month = month.lower()
        if month in ['january', 'february', 'march', 'april', 'may', 'june', 'all']:
            break
        else:
            print("Please choose a month  within first six months")

    # TO DO: get user input for day of week (all, monday, tuesday, ... sunday)
    while True:
        day = input("which day you want to explore ? Please, write day name else type 'all'")
        day = day.lower()
        if day in ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday', 'all']:
            break
        else:
            print("Please choose from the following days ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', "
                  "'saturday', 'sunday', 'all']or write 'all'")
...
def station_stats(df):
    ...
    print("The most frequent combination of start station and end station trip: ", df['the most frequent combination'].mode()[0])
...
def trip_duration_stats(df):
    ...
    print("The total travel time:", df['Trip Duration'], "\n")
...
def user_stats(df, city=None):
    ...
    most_recent = sorted(df.groupby(['Birth Year'])['Birth Year'])[0][0]
